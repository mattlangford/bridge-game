...
struct SimulationContext {
    ...
    double get_coordinate(size_t index) const { return mesh.vertices[index] + get_displacement(index); }
};
...
class Simulator {
   public:
    ...
    void draw() const {
        ...
        for (size_t i = 0; i < context->mesh.triangles.size(); ++i) {
            ...
            if (context->cache.fixed_triangles[i]) {
                ...
                float green = static_cast<float>((kMaxStress - stress) /
    ...
    void set_mesh(common::Mesh mesh) { context = std::make_unique<SimulationContext>(std::move(mesh)); }
