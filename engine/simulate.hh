...
DMatrix generate_D() {
  ...
  const double E =
      30 * 1E9; // youngs modulus N/m^2 (for concrete) (slightly adjusted)
...
Eigen::MatrixXd
generate_mass_matrix(size_t num_displacements,
                     const std::vector<size_t> &vertex_to_displacements,
                     const std::vector<double> &mass) {
  Eigen::MatrixXd M =
      Eigen::MatrixXd::Zero(num_displacements, num_displacements);
  M.setZero();
...
struct SimulationContext {
  ...
  double get_displacement(size_t index) const {
    ...
    return u_index >= state.displacements.size() ? 0.0f
                                                 : state.displacements[u_index];
  }
...
struct TriangleStressHelpers {
  ...
  std::vector<LocalToGlobalKMatrixMapping>
  generate_local_to_global_mapping() const {
    ...
    for (size_t row = 0; row < 3; ++row) {
      for (size_t col = 0; col < 3; ++col) {
        ...
        y_map.global_row_col = {triangle.indices[row] + 1,
                                triangle.indices[col] + 1};
      }
    }
  ...
  void populate_local_stiffness(GlobalKMatrix &global_k) const {
    ...
    // [1] 4.66
    LocalKMatrix local_k =
        kThickness * area() * B.transpose() * generate_D() * B;
    ...
    for (size_t local_row = 0; local_row < 6; ++local_row) {
      for (size_t local_col = 0; local_col < 6; ++local_col) {
        ...
        const auto &vertex_to_displacements =
            context.cache.vertex_to_displacements;
        const size_t dynamic_row = vertex_to_displacements[global_row];
        const size_t dynamic_col = vertex_to_displacements[global_col];
        if (dynamic_row < num_displacements &&
            dynamic_col < num_displacements) {
          global_k.coeffRef(dynamic_row, dynamic_col) +=
              local_k(local_row, local_col);
        }
      }
    }
  }
...
GlobalKMatrix
generate_global_stiffness_matrix(const SimulationContext &context) {
  ...
  for (size_t i = 0; i < context.mesh.triangles.size(); ++i) {
    ...
    TriangleStressHelpers{context.mesh.triangles[i], context}
        .populate_local_stiffness(K);
  }
  return K;
}
...
SimulationContext::SimulationContext(common::Mesh mesh_) {
  ...
  for (size_t i = 0; i < vertex_count; ++i) {
    ...
    cache.vertex_to_displacements[i] =
        num_dynamic_displacements; // store before incrementing
  ...
  cache.mass = generate_mass_matrix(num_dynamic_displacements,
                                    cache.vertex_to_displacements, mesh.mass);
...
State MeshStepper::step(SimulationContext &context) {
  ...
  // [2] Table 9.3 B.3
  Eigen::VectorXd U_accel_next =
      kA0 * (U_next - U) - kA2 * U_vel - kA3 * U_accel;
    ...
    for (size_t i = 0; i < context->mesh.triangles.size(); ++i) {
      ...
      if (context->cache.fixed_triangles[i]) {
        ...
        float red = static_cast<float>(
            stress / kMaxStress); // 0 when stress is 0, 1 when stress is high
        float green = static_cast<float>(
            (kMaxStress - stress) /
            kMaxStress); // 1 when stress is 0, 0 when stress is high
        glColor3f(std::clamp(red, 0.f, 1.f), std::clamp(green, 0.f, 1.f), 0.0f);
      }

      // constexpr float kPxPerMeter = 200;
      constexpr float kPxPerMeter =
          static_cast<float>(BuildingContext::kPxSize) /
          BuildingContext::kBlockSize;
      const auto x = [&](uint8_t i) {
        return kPxPerMeter * context->get_coordinate(triangle.indices[i]);
      };
      const auto y = [&](uint8_t i) {
        return kPxPerMeter * context->get_coordinate(triangle.indices[i] + 1);
      };
...
private:
  ...
  Eigen::Vector3d compute_stress(const common::Triangle &triangle) const {
    ...
    for (size_t i = 0; i < triangle.indices.size(); ++i) {
      ...
      displacements[2 * i + 1] =
          context->get_displacement(triangle.indices[i] + 1);
    }
    return generate_D() *
           TriangleStressHelpers{triangle, *context}.generate_B() *
           displacements;
  }
