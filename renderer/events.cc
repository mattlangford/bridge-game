...
const detail::EventWithMetadata &EventBuilder::get_event_with_metadata() {
  if (std::visit(
          [](const auto &event) { return static_cast<bool>(event.callback); },
          data_.event) == false) {
...
EventState EventHandler::get_state() const { return state_; }
...
void EventHandler::mouse_button_callback(GLFWwindow *window, int button,
                                         int action, int mods) {
  mods_ = mods;
  ...
  for (const auto &handler : handlers_[state_]) {
    if (handler.type != detail::Type::kAny) {
      ...
      if (type == Type::kRelease && action != GLFW_RELEASE)
        continue;
      if ((type == Type::kPress || type == Type::kDoublePress) &&
          action != GLFW_PRESS)
        continue;
    }
    ...
    case GLFW_MOUSE_BUTTON_LEFT:
      if (auto *event =
              std::get_if<detail::LeftButtonMouseEvent>(&handler.event))
        event->callback(window);
      break;
    case GLFW_MOUSE_BUTTON_RIGHT:
      if (auto *event =
              std::get_if<detail::RightButtonMouseEvent>(&handler.event))
        event->callback(window);
      break;
    default:
      break;
  ...
  // Update this here so that next cycle we'll have started the hold, otherwise
  // a single click will be a hold
...
void EventHandler::cursor_position_callback(GLFWwindow *window, double xpos,
                                            double ypos) {
  for (const auto &handler : handlers_[state_]) {
    if ((handler.type != detail::Type::kAny) &&
        (handler.type == detail::Type::kHold) xor is_holding_) {
      continue;
    }
...
void EventHandler::key_callback(GLFWwindow *window, int key, int scancode,
                                int action, int mods) {
  mods_ = mods;
  ...
  for (const auto &handler : handlers_[state_]) {
    ...
    if ((handler.modifiers >= 0) && (handler.modifiers xor mods_) != 0) {
      ...
      // Control or shift modifiers are fine, if that's the key being pressed.
      // Only skip if the key isn't one of those two
      bool control_key =
          key == GLFW_KEY_LEFT_CONTROL || key == GLFW_KEY_RIGHT_CONTROL;
      bool shift_key =
          key == GLFW_KEY_LEFT_SHIFT || key == GLFW_KEY_RIGHT_SHIFT;
      if (((mods_ xor GLFW_MOD_CONTROL) == 0 && !control_key) ||
          ((mods_ xor GLFW_MOD_SHIFT) == 0 && !shift_key)) {
        continue;
      }
    }
...
void route_mouse_button_callback(GLFWwindow *window, int button, int action,
                                 int mods) {
...
void route_cursor_position_callback(GLFWwindow *window, double xpos,
                                    double ypos) {
  if (auto *handler = EventHandler::get()) {
...
void route_key_callback(GLFWwindow *window, int key, int scancode, int action,
                        int mods) {
  if (auto *handler = EventHandler::get()) {
