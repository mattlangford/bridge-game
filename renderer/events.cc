...
auto EventHandler::add(EventState state) -> BuilderDispatch {
    return BuilderDispatch{
        [state, this](const detail::EventWithMetadata &data) { handlers_[state].emplace_back(data); }};
}
...
void EventHandler::mouse_button_callback(GLFWwindow *window, int button, int action, int mods) {
    ...
    for (const auto &handler : handlers_[state_]) {
        if (handler.type != detail::Type::kAny) {
            ...
            if ((type == Type::kHold) xor is_holding_) continue;
            if (type == Type::kRelease && action != GLFW_RELEASE) continue;
            if ((type == Type::kPress || type == Type::kDoublePress) && action != GLFW_PRESS) continue;
        }
        ...
        switch (button) {
            case GLFW_MOUSE_BUTTON_LEFT:
                if (auto *event = std::get_if<detail::LeftButtonMouseEvent>(&handler.event)) event->callback(window);
                break;
            case GLFW_MOUSE_BUTTON_RIGHT:
                if (auto *event = std::get_if<detail::RightButtonMouseEvent>(&handler.event)) event->callback(window);
                break;
            default:
                break;
        }
...
void EventHandler::key_callback(GLFWwindow *window, int key, int scancode, int action, int mods) {
    ...
    for (const auto &handler : handlers_[state_]) {
        if (handler.type != detail::Type::kAny) {
            ...
            if (type == Type::kHold && action != GLFW_REPEAT) continue;
            if (type == Type::kRelease && action != GLFW_RELEASE) continue;
            if ((type == Type::kPress || type == Type::kDoublePress) && action != GLFW_PRESS) continue;
        }
