...
void generate_ridge(BuildingContext &context) {
  ...
  for (size_t w_block = 0; w_block < BuildingContext::kNumWBlocks; w_block++) {
    ...
    context.data[index(BuildingContext::kNumHBlocks / 2)] =
        common::Material::kBrick;
  }

  // On the sides
  for (size_t h_block = 0; h_block < BuildingContext::kNumHBlocks / 2;
       h_block++) {
    auto index = [&](size_t col) { return context.index(col, h_block); };
    context.data[index(0)] = common::Material::kStone;
    context.data[index(1)] = common::Material::kStone;
    context.data[index(BuildingContext::kNumWBlocks - 1)] =
        common::Material::kStone;
    context.data[index(BuildingContext::kNumWBlocks - 2)] =
        common::Material::kStone;
  }
}
...
BuildingContext generate_initial_building_context() {
  ...
  context.data.resize(BuildingContext::kNumHBlocks *
                          BuildingContext::kNumWBlocks,
                      common::Material::kNone);
...
common::Mesh generate_mesh(const BuildingContext &context) {
  ...
  for (size_t index = 0; index < context.data.size(); ++index) {
    ...
    for (const size_t this_index : indices) {
      ...
      // bottom half
      builder.add_triangle({w_end, h_end}, {w_end, h_m}, {w_m, h_end},
                           material);
    }
  }
...
void draw(const BuildingContext &context) {
  ...
  for (uint16_t w_block = 0; w_block < BuildingContext::kNumWBlocks;
       w_block++) {
    for (uint16_t h_block = 0; h_block < BuildingContext::kNumHBlocks;
         h_block++) {
      const common::Material material =
          context.data[context.index(w_block, h_block)];
...
const BuildingContext &Builder::building_context() const {
  return building_context_;
}
...
void Builder::setup_callbacks(EventHandler &handler) {
  ...
  handler.add(EventState::kBuild)
      .key(GLFW_KEY_LEFT_CONTROL,
           [this](GLFWwindow *, int) { set_erase_mode(); });
  handler.add(EventState::kBuild)
      .release()
      .key(GLFW_KEY_LEFT_CONTROL,
           [this](GLFWwindow *, int) { set_erase_mode(false); });

  handler.add(EventState::kBuild)
      .any_type()
      .any_modifier()
      .move([this](GLFWwindow *, double xpos, double ypos) {
...
common::Material Builder::get_drawing_cell() const {
  return get_erase_mode() ? common::Material::kNone
                          : drawing_context_.drawing_cell;
}
...
void Builder::set_mouse_block(double xpos, double ypos) {
  ...
  uint16_t h_block =
      static_cast<uint16_t>(kHeight - ypos) / BuildingContext::kPxSize;
  drawing_context_.mouse_block = std::make_pair(w_block, h_block);
}

void Builder::set_cell_at_mouse_block(const common::Material &new_cell,
                                      bool force) {
  ...
  for (size_t w = 0; w < drawing_context_.cursor_zoom; ++w) {
    for (size_t h = 0; h < drawing_context_.cursor_zoom; ++h) {
      common::Material &cell =
          building_context_
              .data[building_context_.index(w_block + w, h_block + h)];
